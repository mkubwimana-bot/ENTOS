-- ============================================================
-- SME-OS Migration 011
-- Atomic, idempotent RPC to post a (possibly offline-captured) sale.
--
-- Why: the Flutter Quick Sale screen previously posted a sale with three
-- separate client-side inserts (invoice, lines, stock movements). That was
-- not atomic and had no protection against double-posting when a network
-- response was lost and the client retried.
--
-- post_sale_draft records the sale in transaction_drafts keyed by
-- client_reference_id (unique per tenant), then creates the invoice,
-- invoice_lines and stock_movements in a single transaction. Retrying with
-- the same client_reference_id returns the already-posted invoice number
-- instead of creating a duplicate (offline sync idempotency).
-- ============================================================
...
begin
  ...
  -- Permission: mobile quick sale (or general offline use).
  if not (
  ...
  -- ----------------------------------------------------------
  -- Idempotency: has this client_reference_id already posted?
  -- ----------------------------------------------------------
  ...
  -- ----------------------------------------------------------
  -- Create the invoice header (number reserved server-side).
  -- ----------------------------------------------------------
  ...
  -- ----------------------------------------------------------
  -- Lines + stock movements (product data re-fetched for safety).
  -- ----------------------------------------------------------
  ...
  -- ----------------------------------------------------------
  -- Finalize invoice totals and mark the draft as synced.
  -- ----------------------------------------------------------
